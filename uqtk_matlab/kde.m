...
t_star=fzero(@(t)fixed_point(t,N,I,a2),[0 0.1]);
...
l=7;
f=2*pi^(2*l)*sum(power(I,l).*a2.*exp(-I*pi^2*t));
for s=l-1:-1:2
   K0=prod([1:2:2*s-1])/sqrt(2*pi);
   const=(1+(1/2)^(s+1/2))/3;
   time=power(2*const*K0/N/f,(2/(3+2*s)));
   g=power(I,s).*a2.*exp(-I*pi^2*time);
   f=2*pi^(2*s)*sum(g);
end
out=t-power(2*N*sqrt(pi)*f,(-2/5));
end
