function [seed_new, counter, q_eval] = MCMC_samp(seed, gamma, c, num_steps, varargin)
...
q_eval(1) = compute_qoi(seed, 'fix_kle', KLa, pde_data, sigma); % do not need this step, already have it
for i = 1:num_steps
    seed_cand = gamma*seed(end,:) + (sqrt(1-gamma^2) * randn(1, ndim));
    q_eval(i+1) = compute_qoi(seed_cand, 'fix_kle', KLa, pde_data, sigma);
    if q_eval(i+1) > c
